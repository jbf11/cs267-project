...
int main(int argc, char **args)
{
  ...
  PetscReal   dt = 0.1, norm, tol = 1e-12;
  ...
  if (lr) {
    ...
    if (rank == 0) f_vals[0] = 0.5*f_vals[0];
    if (rank == size/2 - 1)  f_vals[ncol-1] = 0.5*f_vals[ncol-1];
  }
  if (rank == size/2 - 1 || rank == size/2) {
    b_rows = (PetscInt*) malloc( (n-2*n_wall) * sizeof(PetscInt));
    b_vals = (PetscReal*) malloc( (n-2*n_wall) * sizeof(PetscReal));
    for (i = 0; i < n - 2*n_wall; ++i) {
      b_rows[i] = (m+rank-size/2)*n + n_wall + i;
      b_vals[i] = (rank == size/2 ? -1 : 1) * k*flx;
    }
